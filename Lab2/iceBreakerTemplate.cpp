...
int main()
{
    ...
    vector<string> qBank;
    bool checkRoster = readFile("2310_F26_Rosters.csv", roster);
    bool checkQBank = readFile("Questions.csv", qBank);
    if(checkRoster && checkQBank){
        cout << "Files read successfully!" << endl;
        bool checkWrite = writeFile("data.csv", roster, qBank);
        if(checkWrite){
 ...
 */
int ranGen(int numQuestions){
    if (numQuestions <= 0) return 0;
    static random_device rd;
    static mt19937 gen(rd());
    uniform_int_distribution<int> dist(0, numQuestions - 1);
    return dist(gen);
}
